...
//Класс позволяет получать, хранить и возвращать по требованию массив 
//XML-ответов о курсе всех доступных валют на заданные даты. 
//Предполагается использовать данные из этого массива для реализации 
//работы объектов, соответствующих отдельной валюте.

class ExchangeRateRepository {

    public $url = 'http://www.cbr.ru/scripts/XML_daily.asp?date_req=';
    public $date_req;
    public $exchange_rates_on_date;

    //Вернуть элемента массива с данными о курсах валют на выбранную дату
    public function returnExchangeRatesOnDate($selected_date) {
        if (!isset($this->exchange_rates_on_date[$selected_date])) {
        ...
        return $this->exchange_rates_on_date[$selected_date];
    }
    ...
    protected function getExchangeRatesOnDate($selected_date) {
        ...
        $this->exchange_rates_on_date[$selected_date] = curl_exec($this->ch);
